...
// InsertProcessedUser records a user along with heuristic analysis flags.
func (d *Database) InsertProcessedUser(username string, createdAt time.Time, totalStars, emptyCount, suspiciousEmptyCount, contributions int,
	flagOriginal, flagNew, flagRecent, analysisResult bool) error {
	query := `
	INSERT OR IGNORE INTO processed_users 
		(username, created_at, total_stars, empty_count, suspicious_empty_count, contributions, flag_original, flag_new, flag_recent, analysis_result)
	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`
	_, err := d.Exec(query, username, createdAt, totalStars, emptyCount, suspiciousEmptyCount, contributions, flagOriginal, flagNew, flagRecent, analysisResult)
	return err
}
