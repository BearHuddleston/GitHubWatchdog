...
func (s *Server) getUsers(page, limit int, sortBy, sortOrder string) ([]UserData, int, error) {
	...
	if err := s.populateUserCategories(users); err != nil {
		return nil, 0, err
	}
...
func (s *Server) populateUserCategories(users []UserData) error {
	if len(users) == 0 {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("querying user flags: %w", err)
	}
	defer rows.Close()

	flagsByUser := make(map[string][]string, len(usernames))
	for rows.Next() {
		var username, flag string
		if err := rows.Scan(&username, &flag); err != nil {
			return fmt.Errorf("scanning user flag row: %w", err)
		}
		flagsByUser[username] = append(flagsByUser[username], flag)
	}
	if err := rows.Err(); err != nil {
		return fmt.Errorf("iterating user flag rows: %w", err)
	}

	for i := range users {
		users[i].Categories = deriveUserCategories(userCategoryInput{
			CreatedAt:            users[i].CreatedAt,
			TotalStars:           users[i].TotalStars,
			EmptyCount:           users[i].EmptyCount,
			SuspiciousEmptyCount: users[i].SuspiciousEmptyCount,
			Contributions:        users[i].Contributions,
			IsSuspicious:         users[i].AnalysisResult,
			HeuristicFlags:       flagsByUser[users[i].Username],
		})
	}
